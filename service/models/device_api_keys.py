...
class DeviceApiKeyStatus(Enum):
    """Enumeration of valid DeviceApiKey statuses"""
...
class DeviceApiKeys(db.Model):
    """
    Class that represents a DeviceApiKey
    ...
    status = db.Column(
        db.String(),
        nullable=False,
        default=DeviceApiKeyStatus.CREATION_REQUEST_RECEIVED.value,
    )
    ...
    def __repr__(self):
        return f"<DeviceApiKey {self.device_id} id=[{self.id}]>"

    def create(self):
        """
        Creates a DeviceApiKey to the database
        """
    ...
    def update(self):
        """
        Updates a DeviceApiKey to the database
        """
    ...
    def delete(self):
        """Removes a DeviceApiKey from the data store"""
    ...
    def serialize(self):
        """Serializes a DeviceApiKey into a dictionary"""
        return {
            "id": self.id,
    ...
    def deserialize(self, data: dict):
        """
        Deserializes a DeviceApiKey from a dictionary
        Args:
            data (dict): A dictionary containing the DeviceApiKey data
        """
        try:
            self.device_id = data["device_id"]
            self.status = data.get(
                "status", DeviceApiKeyStatus.CREATION_REQUEST_RECEIVED.value
            )
        except AttributeError as error:
            raise DataValidationError("Invalid attribute: " + error.args[0]) from error
        except KeyError as error:
            raise DataValidationError(
                "Invalid DeviceApiKey: missing " + error.args[0]
            ) from error
        except TypeError as error:
            raise DataValidationError(
                "Invalid DeviceApiKey: body of request contained bad or no data "
                + str(error)
            ) from error
    ...
    @classmethod
    def all(cls) -> list:
        """Returns all of the DeviceApiKeys in the database"""
        logger.info("Processing all DeviceApiKeys")
        return cls.query.filter(cls.active == True)

    @classmethod
    def find(cls, id: int):
        """Finds a DeviceApiKey by it's ID

        :param id: the id of the DeviceApiKey to find
        :type id: int
    ...
    def find_or_404(cls, id: int):
        """Find a DeviceApiKey by it's id
    ...
    def find_by_device_id(cls, device_id: int) -> list:
        """Returns all DeviceApiKeys with the given device_id

        :param device_id: the device_id of the DeviceApiKeys you want to match
        :type device_id: int

        :return: a collection of DeviceApiKeys with that device_id
        :rtype: list
    ...
    def find_by_status(cls, status) -> list:
        """Returns all DeviceApiKeys by their status

        :param status : DeviceApiKeyStatus Enum
        :type DeviceApiKeyStatus: Enum

        :return: a collection of DeviceApiKeys that are available
        :rtype: list
